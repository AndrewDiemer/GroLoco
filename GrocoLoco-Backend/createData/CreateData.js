...
function CreateData() {
  ...
  // DeleteData();
  // var Products = ["apple cider","apples","apple pie"];
  // var Position = 1;
  // var UPC = "4444444444"   
  // for(var i = 0; i < Products.length; i++) {

  //    CreateGroceryItem(UPC, Products[i], "ruthies apple cider", "liquor", "11L", "01", Position, 0.8);

  //    CreateBlockCoordData(UPC, 5, 10, 5, 5);

  //    Position++;
  //    UPC++;
  // }

  // FindCoordinatesForItems();
...
function DeleteData() {
  ...
  BlockCoordinates.remove({
  }, function(err, blockcoordinates){
      if(err) res.send(err);
  });

  ItemWidth.remove({ 
  }, function(err, ItemWidth){
      if(err) res.send(err);
  });
}
...
function CreateGroceryItem(UPC, Description, POSDescription, SubCategory, Aisle, AisleShelf, Position, Width) {

  console.log('Creating Grocery Data');
...
function FindCoordinatesForItems() {
  ...
  GroceryItem.find({ }, function(err, items){
    ...
    if(items) {
        for(var i = 0; i < items.length; i++) { // for(var i = 0; i < 1; i++) {        
        
          console.log('found item and now need to find coordinates');
                  ...
                  GroceryItem.find({'AisleShelf' : AisleShelf}).sort('Position').exec(function(err, groceryItems) {
                    if(err) {
                        ...
                        asyncLoop( 0, function() {
                            ...
                            if(dist==0) {
                              console.log('ERROR - shelf distance was 0');
                              dist = 1;
                            }
