...
- (void)buttonMethod:(UIButton *)sender
{
    ...
    [GLNetworkingManager setUserLocation:annotation.title longitude:@(annotation.coordinate.longitude) latitude:@(annotation.coordinate.latitude) completion:^(NSDictionary *response, NSError *error) {
        if (error){
            [self showError:error.localizedDescription];
        }
        else{
